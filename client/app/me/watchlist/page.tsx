...
async function fetchListItems(listRows: Array<{ item_type: string; wp_post_id: number }>) {
  ...
  const [movieData, seriesData, episodeData] = await Promise.all([
    movies.length > 0
      ? wpFetchMovies({
          perPage: 100,
          // WordPress doesn't support include directly, so we'll filter client-side
        })
...
export default async function WatchlistPage() {
  ...
  let listRows;
  try {
    listRows = await meApi.getLists({ list_type: 'watchlist' }, cookieHeader);
  } catch (error) {
    redirect('/login');
  }
  ...
  if (listRows.length === 0) {
    return (
      <PageContainer title="Watchlist" description="Movies and series you want to watch">
        <div className="rounded-2xl border border-black/10 bg-white p-8 text-center dark:border-white/10 dark:bg-slate-800">
          ...
          <div className="mt-2 text-sm text-slate-600 dark:text-slate-400">
            Add movies and series to your watchlist to see them here.
          </div>
          <Link
            href="/movies"
      ...
      <div className="space-y-8">
        ...
        {movieCards.length === 0 && seriesCards.length === 0 && filteredEpisodes.length === 0 && (
          <div className="rounded-2xl border border-black/10 bg-white p-8 text-center dark:border-white/10 dark:bg-slate-800">
            ...
            <div className="mt-2 text-sm text-slate-600 dark:text-slate-400">
              The items in your watchlist may have been removed from WordPress.
            </div>
          </div>
        )}
      </div>
